...
export default function TambahData({ onAddData }) {
  ...
  const [mapel, setMapel] = useState(""); // Mengganti 'jurusan' dengan 'mapel'
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const newData = {
        nama,
        nik,
        mapel, // Menambahkan field mapel
        gender, // Menambahkan field gender
      };

      // Mengirim data baru ke backend (pastikan endpoint sudah benar)
      const response = await axios.post("http://localhost:3030/guru", newData);
  ...
  return (
    ...
    >
      <StyledPaper>
        <FormTitle>✨ Tambah Guru 🎓</FormTitle>
